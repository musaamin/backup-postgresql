...
func main() {
	...
	if len(os.Args) < 3 {
		fmt.Println("Usage:")
		fmt.Println("  To initialize a configuration file: ./backup-postgresql init <file>")
		fmt.Println("  To perform PostgreSQL backup: ./backup-postgresql export <file>")
		fmt.Println("  To execute rclone sync: ./backup-postgresql rclone <file>")
		return
	}
...
func exportPostgreSQL(configFile string) {
	...
	cmd := exec.Command("pg_dump", "-h"+dbHost, "-p"+dbPort, "-U"+dbUser, dbName)
	...
	if err != nil {
		...
		if exitError, ok := err.(*exec.ExitError); ok {
			...
			if exitStatus == 2 {
				...
				errMsg = fmt.Sprintf("Error running pg_dump: Exit Status %d", exitStatus)
			}
		} else {
			errMsg = "Error running pg_dump: " + err.Error()
		}
		fmt.Println(errMsg)
		return
	}
